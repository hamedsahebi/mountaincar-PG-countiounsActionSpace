...
class PolicyModel:
    def __init__(self, ft, D, hidden_layer_sizes_mean=[], hidden_layer_sizes_var=[]):
        ...
        for M2 in hidden_layer_sizes_mean:
            self.mean_layers.append(HiddenLayer(M1,M2))
            M1 = M2
        ...
        self.actions = tf.compat.v1.placeholder(
            tf.float32, shape=(None,), name='actions')
        ...
        norm = tf.compat.v1.distributions.Normal(mean, std)
    ...
    def copy(self):
        clone = PolicyModel(
            self.ft, self.D, self.hidden_layer_sizes_mean, self.hidden_layer_sizes_mean)
...
def play_one(env, pmodel, gamma,initial_state=None):
...
def play_multiple_episodes(env, T, pmodel, gamma, print_iters=False,initial_state=None):

    totalrewards = np.empty(T)

    for i in range(T):
        totalrewards[i] = play_one(env, pmodel, gamma,initial_state)
...
def plot_cost_to_go(env, estimator,pmodel,gamma, num_tiles=5):
    x = np.linspace(
    ...
    Z = np.apply_along_axis(
        lambda _: estimator(env,3, pmodel, gamma, print_iters=False,initial_state=_), 2, np.dstack([X, Y]))
...
def main():
    ...
    avg_totalrewards = play_multiple_episodes(
        env, 100, pmodel, gamma, print_iters=True)
    print("avg reward over 100 episodes with best models:", avg_totalrewards)

    plt.plot(totalrewards)
    plt.title("Rewards")

    plot_cost_to_go(env,play_multiple_episodes,pmodel,gamma,num_tiles=5)
    plt.show()
